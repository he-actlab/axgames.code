...
basedir = os.environ['GAME_EMBOSS']



def runEmboss(filename):
	print
	...
	os.system('java -cp ' + basedir + '/emboss.jar Emboss.RgbImage ' + basedir + '/images/' + rgbfilename + ' ' + basedir + '/rgboutput/' + name + ' nrmse')
	print "emboss done ... (nrmse)"
	
	#os.system('java -cp ' + basedir + '/emboss.jar Emboss.RgbImage ' + basedir + '/images/' + rgbfilename + ' ' + basedir + '/rgboutput/' + name + ' psnr')
	#print "Emboss done ... (psnr)"
...
def main():
	...
	for chunk in range(0, (len(newfiles) / PARALLEL_WIDTH) + 1):
		pool = [Process(target=runEmboss, args=(str(newfiles[i + chunk * PARALLEL_WIDTH]),)) for i in range(min(PARALLEL_WIDTH * (chunk+1), len(newfiles)) - PARALLEL_WIDTH * chunk)]
